...
- **Modo despacho (multi-máquina)** — si se activó en calibración: cada sesión del plan se publica como **GitHub Issue** con título `[S<NN>] <objetivo en una frase>`, cuerpo = la ficha completa (🎯 planteamiento · 🛠️ método · ✅ criterios verificables · checklist de pendientes · ⛓️ prerrequisitos con links a sus issues) y labels `encargo` + `sesion-NN` + `maquina/<nombre>` (creá los labels que falten con `gh label create`). **Asignar = poner el label de máquina; reasignar = cambiarlo.** En cada máquina trabajadora el trabajo se toma con `/proximo-encargo` (mismo plugin), que respeta dependencias y reclama el issue antes de arrancar. **Issue cerrado ≠ HECHO**: en la re-auditoría se verifica en código igual que un tick ámbar — si no pasa, reabrí el issue con el hallazgo como comentario. El tracker linkea cada sesión a su issue y sigue siendo el mapa; los issues son la cola de trabajo. Al re-auditar, sincronizá ambos: pendiente cerrado y verificado → issue cerrado con comentario de verificación; sesión nueva en el plan → issue nuevo.
