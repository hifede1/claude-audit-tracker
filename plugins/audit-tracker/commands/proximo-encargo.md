...
2. **Buscá el encargo.** `gh issue list --label encargo --assignee @me --state open` (o `--label "maquina/<nombre>"` en modo etiqueta) → ordená por número de sesión (label `sesion-NN` más bajo primero).
   **Sin resultados NO significa que no haya trabajo — falta un paso antes de reportar.** Traé la cola entera y separala en dos:
   ```
   gh issue list --label encargo --state open --json number,title,assignees,labels
   ```
   - **(a) de otros** — con assignee o label `maquina/*` ajenos: informá quiénes tienen cola.
   - **(b) HUÉRFANOS** — sin assignee **y** sin label `maquina/*`: están abiertos, tomables, y **ningún ejecutor los ve**. Reportalos por número como hallazgo, con todas las letras: *no es que no haya trabajo, es que hay trabajo sin dueño*. Están mal publicados — ver «Asignación» en `audit-tracker.md`, modo despacho.

   **Y NO te los auto-asignes.** Asignar es del despachador. Si te adjudicás un huérfano, el reparto de la cola pasa a decidirse por orden de llegada y deja de ser decisión de nadie — que es peor que la ceguera que acabás de detectar. Reportás, y esperás.
...
5. **Leé las referencias ANTES de escribir código.** Si la ficha lista 📚 referencias (`docs/references/`, `docs/business/`), leelas primero: definen cómo se hace bien esta clase de feature. Barré también POR TRIGGER: toda referencia de `docs/references/` cuyo frontmatter `triggers` matchee el tema del encargo se lee aunque la ficha no la linkee. Si el encargo es territorio nuevo (proveedor externo, patrón que el repo no usa aún) y NO hay referencia, **generala primero**: investigación en fuentes oficiales, destilada en `docs/references/<tema>.md` (patrones, tradeoffs, citas, fecha y frontmatter `triggers`) y commiteada en la misma rama — el próximo que toque el tema no arranca de cero. Si una referencia existente contradice lo que ves en las fuentes actuales, marcala como desactualizada en el issue: no construyas sobre referencia vencida. Altas, refrescos y vencimientos llegan a la pestaña Referencias del tracker vía re-auditoría.
...
- **Freno anti-loop**: mismo error tres veces, o un gate que no pasa tras varios intentos = estás trabado. No insistas a ciegas: comentá en el issue qué intentaste, qué falla y tu hipótesis, y pedí ayuda o desistí 🔓. Trabado que insiste ≠ progreso.
